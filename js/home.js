...
const categoryVideo = {
  ...
  movies.forEach((movie, index) => {
    ...
    if (container) {
      ...
      row.innerHTML += `
        <div class="modal fade" id="${modalId}" tabindex="-1" aria-hidden="true">
          <div class="modal-dialog modal-fullscreen">
            <div class="modal-content modal-transparent">
              <div class="modal-header my-btn-close p-4">
                <button type="button" id="btnModal" class="btn-close btn-close-white" data-bs-dismiss="modal" aria-label="Close"></button>
              </div>
              <div class="modal-body p-0 m-0">
                <div class="video-container">
                  <iframe width="1920" height="823" src='https://www.youtube.com/embed/${movie.link}' title="${movie.title}" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerpolicy="strict-origin-when-cross-origin" allowfullscreen></iframe>      
                </div>
              </div>
            </div>
          </div>  
        ...
        <div class="col-12 col-sm-6 col-md-4 col-xl-3">
          <div class="content w-100">
            <img src="${movie.img}" class="w-100 img-content" alt="Capa do video ${movie.title}">
            <div class="container card-description bg-black ps-3">
              <div class="row align-items-end">
                <div class="col m-auto">
                  <a href="${movie.link}" target="_blank" data-bs-toggle="modal" data-bs-target="#${modalId}" class="text-decoration-none text-light d-flex align-items-center fs-2 mb-3">
                    <i class="bi bi-play-circle text-light fs-1 m-1 d-flex"></i>
                    <p class="video-title mb-0 d-flex align-items-center justify-content-start">${movie.title}</p>
                  </a>
                </div>
              </div>
            </div>
      ...
      console.error(`Categoria não encontrada: ${movie.category}`);
    } 
  });


// Stop video playback when clicking on btn-close
...
// Show video description on click (mobile)
document.querySelectorAll('.img-content').forEach((img) => {
  img.addEventListener('touchstart', (event) => {
    const card = img.closest('.content');
...
// Hide description when tapping outside of it (mobile)
document.body.addEventListener('touchstart', (event) => {
  if (!event.target.closest('.card-description') && !event.target.classList.contains('img-content')) {
    document.querySelectorAll('.card-description').forEach((desc) => {
      desc.style.display = 'none';
    });
  }
